...
**`{orm}TryDb` factories** (`db-result/drizzle`, `db-result/kysely`) are
per-ORM DX layers that wrap an ORM client so every return shape carries the
E-track — they are not union entry points (the protocol still comes from your
driver import). A query builder passed as a value narrows the union further
per ORM (Kysely, Drizzle) — see [shapes](./shapes.md). Prisma has no wrapper:
every delegate method is one-shot (its `PrismaPromise` memoizes the executed
query), so Prisma calls use the thunk form — full union, retry by
re-invocation, P-codes classified — see [transactions](./transactions.md).
