...
- **`{orm}TryDb` factories** — `db-result/drizzle`, `db-result/kysely`, `db-result/prisma` — commit-to-Result wrappers that E-track every builder chain, transaction, and delegate call without thunks at the call site. Kysely chains keep precise row types; Prisma delegate calls resolve with the full union (no builder types to probe).
- **Retry doctrine** — transient tags (`db/deadlock`, `db/lock-timeout`, `db/serialization-failure`, `db/io-error`, `db/connection-error`) auto-retry with jittered backoff; one-shot tags (`db/unique-violation`, `db/foreign-key-violation`, …) never retry. Override per call or per driver.
- **Type-level shape lattice** — "no lying types": a tag excluded from a shape's union is genuinely impossible for that shape on that driver, verified adversarially (60 probe attacks; five lattice lies found and closed before release).
